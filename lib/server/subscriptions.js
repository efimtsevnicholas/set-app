...
export async function getStripeSubscriptions(db, userId) {
  const { data, error } = await db.from('subscriptions')
    .select('id,provider_customer_id,provider_subscription_id,status,plan_code,current_period_ends_at,trial_ends_at,cancel_at_period_end,provider,created_at,updated_at')
    .eq('user_id', userId)
    .eq('provider', 'stripe')
