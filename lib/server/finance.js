export function computeDocumentTotals(items=[]){
  const normalized=items.map((x,i)=>({description:String(x.description||'Service').trim(),quantity:Number(x.quantity||1),unit_price:Number(x.unit_price||0),vat_rate:Number(x.vat_rate||0),sort_order:i}));
